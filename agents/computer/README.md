...
### 5. 登录凭据 (account_login)

使用 **`account_login`** 从本地配置填充时，**不得**在 `tool_args` 中传入明文密码或登录用户名；应传 **`system`**（系统/站点名）与可选 **`user_label`**（用户别名，与配置表中一致），再加上序号或坐标等目标字段。

**推荐（Web 界面）**：**Settings → External Services → Computer logins** 中维护 **`system`、`user_label`、`username`、`password`**，**Save accounts**。每轮视觉注入会在 prompt 中附带 **Saved login accounts** 元数据列表（无密码），供模型选择 `system` / `user_label`。

**文件格式**：`usr/computer_credentials.json` 使用 **`{"version": 1, "accounts": [ ... ]}`**；旧版 **`profiles: { id: {...} }`** 在读取时会自动按内存迁移。示例见 `agents/computer/computer_credentials.example.json`。可选 **`computer_credentials_path`**（`usr/settings.json`）。

工具方法：**`account_login:fill_at_indices`** 或 **`fill_at_coordinates`**；`fill` 可为 **`username`** / **`password`** 或省略以一次填多项。详见 `prompts/agent.system.tool.account_login.md`。
...
2. 模型应输出 `tool_name` 如 `mouse:click_index`、`composite_action:type_text_at_index`、`account_login:fill_at_indices`，以及 `tool_args`（`index` 必填，`type_text_at_index` 需带 `text`；**登录**用 **`system`** + 可选 **`user_label`**，见 §5）。
