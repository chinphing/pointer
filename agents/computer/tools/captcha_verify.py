...
# 单次移动总时长范围（秒），再按路径点数均分得到每步 interval
...
def _move_along_path(
    ...
    """沿路径逐点移动。若传 total_time 则整段移动在 total_time 内完成（每步 interval=total_time/点数）；否则用固定 interval。"""
    import pyautogui
    if not points:
        return
    if total_time is not None and total_time > 0:
        interval = total_time / len(points)
...
def _do_drag(
    ...
    if is_slider and screen_points:
        ...
        offset_px = max(0, int(_settings_mod.get_settings().get("captcha_slider_offset_px") or 10))
        screen_points = list(screen_points)
        screen_points[-1] = [int(screen_points[-1][0]) + offset_px, int(screen_points[-1][1])]
    _captcha_log(tool.agent, "模拟人工操作")
    # 多点串联成完整贝塞尔轨迹，再 down -> 沿轨迹 move -> up
